...
class TargetPractice:
    ...
    def run_game(self):
        ...
        while True:
            ...
            if self.game_active:
                self.ship.update()
                self._update_bullets() #need to switch func call here to '.bullets.update()
                self.target.update()
                
            self._update_screen() #need to switch func call here to '.screen.update()
            self.clock.tick(60) #60 fps limit
                
            #Get rid of bullets that have disappeared.
            for bullet in self.bullets.copy(): #Made a copy of bullets to modify without altering the original list
                if bullet.rect.bottom <= 0: #if the shape touchs the bottom
